...
public Action Command_ShowPermissions(int client, int args) {
    ...
    for (int i = 0; i < g_Commands.Length; i++) {
        ...
        switch (p) {
            case Permission_All: Format(permission, sizeof(permission), "all");
            case Permission_Captains: Format(permission, sizeof(permission), "captains");
            case Permission_Leader: Format(permission, sizeof(permission), "leader");
            case Permission_Admin: Format(permission, sizeof(permission), "admin");
        }
