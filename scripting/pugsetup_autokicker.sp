...
public void OnPluginStart() {
    ...
    g_AutoKickerEnabledCvar = CreateConVar("sm_pugsetup_autokicker_enabled", "1", "Whether the autokicker is enabled or not");
    g_KickMessageCvar = CreateConVar("sm_pugsetup_autokicker_message", "Sorry, this pug is full", "Message to show to clients when they are kicked");
    g_KickNotPickedCvar = CreateConVar("sm_pugsetup_autokicker_kick_not_picked", "1", "Whether to kick players not selected by captains in a captain-style game");
    g_KickWhenLiveCvar = CreateConVar("sm_pugsetup_autokicker_kick_when_live", "1", "Whether the autokicker kicks newly connecting clients during live matches when there are already full teams");
    g_TimeToReadyCvar = CreateConVar("sm_pugsetup_autokicker_ready_time", "0", "The time (in seconds) clients have to ready up before being kicked, set to 0 to disable.");
    g_TimeToReadyKickMessageCvar = CreateConVar("sm_pugsetup_autokicker_ready_time_kick_message", "You failed to ready up in time", "Message clients recieve when kicked for not readying up in time.");
    g_UseAdminImmunityCvar = CreateConVar("sm_pugsetup_autokicker_admin_immunity", "1", "Whether admins (defined by pugsetup's admin flag cvar) are immune to kicks");
