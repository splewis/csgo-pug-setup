...
public InitialChoiceHandler(Handle:menu, MenuAction:action, param1, param2) {
    if (action == MenuAction_Select) {
        ...
        if (choice == InitialPick_Player) {
            PluginMessage("\x03%N \x01has elected to get the \x04first player pick.", g_capt1);
            SideMenu(g_capt2);
...
public SideMenuHandler(Handle:menu, MenuAction:action, param1, param2) {
    if (action == MenuAction_Select) {
        ...
        if (choice == SideChoice_CT) {
            PluginMessage("\x03%N \x01has picked \x04CT \x01first.", client);
            teamPick = CS_TEAM_CT;
...
public PlayerMenuHandler(Handle:menu, MenuAction:action, param1, param2) {
    if (action == MenuAction_Select) {
        ...
        if (selected > 0) {
            ...
            if (client == g_capt1)
                PluginMessage("\x03%N \x01has picked \x03%N", client, selected);
            else
                PluginMessage("\x06%N \x01has picked \x06%N", client, selected);
